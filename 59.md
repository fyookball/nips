...
In order to obtain a shared secret between Alice and Bob, the following steps should be performed:

1. Calculate the ECDH product based on the 33 byte public key (same as NIP-04)
2. Hash the result using SHA-256
3. Create a salt using the common phrase "A decentralized social network with a chance of working." Encode this string using ascii values, then convert to a hex string, and hash the result using SHA-256.
4. Apply the HKDF using the salt, the hashed DH result from step 2, and the SHA-512 hashing algorithm.
5. Derive a 32-byte key using the bytestring **b"nostr1"** as the 'info' input to HKDF.

After the common secret has been established, it can be used with AES-256-GCM. Encryption should produce a ciphertext, but also a nonce and tag. The tag is used for message authentication.  All 3 (ciphertext, nonce, and tag) are needed for decryption, and should be passed to the recipient.  

A sample implementation can be found here:  https://github.com/Electron-Cash/Nostron/blob/nip04research/nip04research/EncryptedMessaging.py
